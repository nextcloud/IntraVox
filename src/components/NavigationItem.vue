...
export default {
  ...
  computed: {
    ...
    selectedPage() {
      if (!this.localItem.pageId) return null;
      const page = this.pages.find(p => p.id === this.localItem.pageId);
      return page ? { value: page.id, label: page.title } : null;
    },
    selectedTarget() {
  ...
  methods: {
    ...
    updatePageLink(option) {
      if (option) {
        this.localItem.pageId = option.value;
