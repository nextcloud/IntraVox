...
- **Feed widget: Nextcloud source type** — Native integration with Nextcloud instances (local or remote). Content types: Activity and Notifications. Local instances use Bearer token auth; remote instances use OAuth2 Authorization Code flow with per-user tokens. Each user sees only their own data — fully security-trimmed. Focuses on organizational context (activity streams) rather than personal productivity (which is the Nextcloud Dashboard's role). Reuses existing LMS OAuth2 infrastructure (`FeedReaderService.php`, `LmsOAuthService.php`)
