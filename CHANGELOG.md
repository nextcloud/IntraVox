...
- **Feed widget: Nextcloud source type** — Native integration with Nextcloud instances (local or remote). Three personalized content types: My Activity, My Shares, My Notifications. Local instances use Bearer token auth; remote instances use OAuth2 Authorization Code flow with per-user tokens. Each user sees only their own data — fully security-trimmed. Reuses existing LMS OAuth2 infrastructure (`FeedReaderService.php`, `LmsOAuthService.php`)
