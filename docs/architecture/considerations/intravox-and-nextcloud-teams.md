...
| **Scope** | One team | Whole organization, a department — or, with multi-site, one team |
...
4. **We intend to render inside the team page, not only be listed on it.** Once IntraVox supports multiple sites, a team's own site is the natural thing to show where the team page currently shows an empty folder or a markdown file.

Principle 3 is the one that decides feature requests, so most of this document explains it. Point 4 is direction rather than a shipped feature, and the section on it says what still has to be true first.
...
The display side needs nothing from Nextcloud. The team page groups resources by provider and uses the name and icon the provider itself supplies, so an IntraVox heading appears without an upstream change.
...
## Where this is heading: a team's site inside the team page

The sections above describe what holds today, where IntraVox is one intranet per instance and a team page can at best link out to it. Two developments change that, and they point at the same place.

**Multi-site.** IntraVox is moving from one site per instance to one site per Team folder — "every Team folder can have its own IntraVox homepage". At that point "an IntraVox page belonging to a team" stops being an approximation and becomes a real, addressable thing: the team's site.

**Inline rendering already exists on the team page.** A team page today is a markdown file in the Team folder, and the Teams app mounts the Text editor inline to render it, through an embed API that Text exposes for exactly this purpose. The pattern — an app publishes a mount function, Teams calls it — is proven and in production. It is simply called for one app by name.

Put together: a team that has an IntraVox site should be able to land on it. Where the team page currently offers an empty folder or a markdown file, the richer answer is a page built with layouts, widgets, and the team's own content — with draft/publish and ACLs behaving as they do everywhere else in IntraVox.

Three things have to be true before that works, in this order:

1. **A site registry.** Multi-site has to exist; a team's site has to be a thing that can be named and resolved. This is IntraVox's own work and everything else waits on it.
2. **An embed API.** IntraVox must expose a mount function comparable to Text's, so a host can render a page into an element it owns and tear it down cleanly. Also IntraVox's own work, and useful beyond Teams.
3. **An upstream change in Teams.** Resource-provider entries are external links, and the landing-page logic deliberately skips anything with an `href` — a provider entry can never be a landing page by design. Making a third-party page a first-class tab requires a change in the Teams frontend.

Step 3 is the only part we do not control, and the honest framing of it is not "Nextcloud should add IntraVox" but "make generic what you already do for Text". The server side is already permissive: the tab order accepts arbitrary ids and validates none of them, so half the infrastructure exists. The barrier is a hardcoded candidate list in the frontend.

Until all three land, the resource provider is the integration, and it is worth doing on its own merits.
...
**Forms has no Teams answer today.** Nextcloud Forms does not implement a team resource provider and has no open issue to do so; it appears only as an unchecked item on the [team-owned resources](https://github.com/nextcloud/circles/issues/2644) list. So for forms, "use Teams instead" is not yet available advice, and the URL-template work in §2 is what serves that case in the meantime — a template supplied once rather than a connector hardcoded per app.

That is the short-term answer. The longer-term one is [FormVox](https://apps.nextcloud.com/apps/formvox), which stores each form as a file with a stable file id and generates its own links — so it can report itself to a team the same way Talk does, rather than being reconstructed from the outside. A form that belongs to a team showing up on the team page next to that team's site is the same principle as everything else here: the app that owns the content owns the link to it.
...
It also sets the direction. As IntraVox becomes multi-site, a team's site is the obvious thing to show on a team page that today offers an empty folder — rendered inline, the way the Teams app already renders a Text document. Getting there is mostly our own work, and the one part that is not is a reasonable thing to ask upstream: make generic what you already do for one app.
